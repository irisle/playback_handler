...
{
   static char *jsn_TYPEN[] = { "FALSE", "TRUE", "NULL", "INTEGER", "REAL", 
                                "STRING", "NODE", "ARRAY", "?" };
   char *p;
   jsn_t *f = node->child;
   while(f)
   {
      switch(f->type)
      {
         case jsn_INTEGER: p = (char*)-12345678; break;
         case jsn_REAL   : p = (char*)-12341234; break;
      ...
      if(p == (char*)-12345678)
         xbuf_xcat(buf, "%*C Type: <b style=\"color:#4080d0\">%s</b> "
                        ...
                        "Value: <b style=\"color:#4080d0\">%lu</b><br>", 
...
int main(int argc, char *argv[])
{
   ...
   jsn_add_integer(user, "created",  now);
   ...
   if(user)
   {
      ...
      // ----------------------------------------------------------------------
      search = jsn_updt(search, (ulong)"abc");
      if(search)
      ...
      time_t stamp = (time_t)search->integer; // the item numeric value
   ...
   if(user)
   {
      ...
      double ids[] = {1.0, 1.1, -1.00001, 9.99, 9.99185, 54.1025, 45, -4578.0};
      jsn_t *array = jsn_add_array(user, "IDs", 0);
      
      int i = 0;
      while(i < (sizeof(ids) / sizeof(double)))
      {
         jsn_add_real(array, "", ids[i]); // 'name' can be "" or null
         i++;
      }
      ...
      if(search)
         xbuf_xcat(reply, "IDs[%3d] = %f<br>", index, search->real);
      else
      ...
      search = jsn_byindex(array, index);
      if(search)
         xbuf_xcat(reply, "IDs[%3d] = %f<br><br>", index, search->real);
      else
   ...
   if(array)
   {
      ...
      jsn_add_integer(node, "created",  now);
