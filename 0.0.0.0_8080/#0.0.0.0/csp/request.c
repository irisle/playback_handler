...
int main(int argc, char *argv[])
{
   ...
   // -------------------------------------------------------------------------
   xbuf_ctx buf;
   long code = 0; // assume cached file is not there, or not recent enough
   char *csp_root = (char*)get_env(argv, CSP_ROOT); // get ".../csp/" path
   char szfile[1024];
   ...
   if(!code || code == 20) // cached file not found (or not up-to-date)
   {
      ...
      if(code == 200 && get_rate(buf2.ptr, "USD"))
      {
         ...
         xbuf_init  (&buf);
         xbuf_ncat  (&buf, buf2.ptr, buf2.len);
      }
      else
      {
   ...
   if(buf.len)
   {
      ...
      //double rEUR = 1.;
      double rUSD = get_rate(buf.ptr, "USD");
      double rCHF = get_rate(buf.ptr, "CHF");
      //double rGPB = get_rate(buf.ptr, "GPB");
