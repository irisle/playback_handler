...
// float tab[] = {-1.5, .8, .3, .5, .7, .1, .6, .9, .5, .3, .5, .2, .3, .1};
   ...
   int len = gif_build(gif, img.bmp, img.w, img.h, (u8*)pal, 1 << img.bbp, -1, 0);
...
int main(int argc, char *argv[])
{
   ...
   {
      if(draw_chart(reply, 200, 100, types[i]) != 200)
      {
         reply->len = 0;
         *reply->ptr = 0;
         return 503; // error
